import pandas as pd
# LabelEncoder - atribue um número para cada texto
from sklearn.preprocessing import LabelEncoder
# DecisionTreeRegressor - O modelo que vamos usar (Cria uma árvore de decisão que aprende regras simples a partir dos dados. Ela divide os dados em ramos (como um fluxograma) até conseguir prever o valor numérico desejado.)
from sklearn.tree import DecisionTreeRegressor
# train_test_split - separar em dados de treino e teste
from sklearn.model_selection import train_test_split
# Mean Absolute Error e R2 - para calcular precisão/acuracia do modelo, não podemos usar accuracy_score para modelos de regressão
from sklearn.metrics import mean_absolute_error
from sklearn.metrics import r2_score
# matplotlib - serve para mostrar gráficos
import matplotlib.pyplot as plt
# pickle - para guardar o modelo em um arquivo .pkl, não precisa instalar, já vem com o python :D
# também pode ser usado o joblib no lugar do pickle mas como só sei usar pickle vou usar ele mesmo
import pickle

# Função para calcular o erro (MAE) de um modelo com certo número de folhas
def calcular_mae(max_de_folhas, x_treino, x_teste, y_treino, y_teste):
    modelo_de_iowa = DecisionTreeRegressor(max_leaf_nodes=max_de_folhas, random_state=0)
    modelo_de_iowa.fit(x_treino, y_treino) # Treina o modelo
    previsao_modelo_de_iowa = modelo_de_iowa.predict(x_teste) # Faz previsões no conjunto de validação
    mae = mean_absolute_error(y_teste, previsao_modelo_de_iowa) # Calcula o erro médio absoluto
    r2 = r2_score(y_teste, previsao_modelo_de_iowa)

#    for previsto, real in zip(previsao_modelo_de_iowa[:10], y_teste[:10]):
#        print(f"Valor Previsto: ${previsto:.2f}")
#        print(f"Valor Real Observado: ${real:.2f}\n")

    # Gerar o gráfico sobre a precisão do modelo
    plt.figure(num="Comparação de Preços — Iowa Housing")  # muda o nome da janela
    plt.scatter(y_teste, previsao_modelo_de_iowa)
    plt.plot([y_teste.min(), y_teste.max()],
            [y_teste.min(), y_teste.max()],
            color='red', linestyle='--')
    plt.xlabel("Valor Real")
    plt.ylabel("Valor Previsto")
    plt.title(f"Comparação entre Valores Reais e Previstos (Modelo de {max_de_folhas} folhas)")
    plt.show() # block=False para não pausar o programa
    return mae, r2, modelo_de_iowa
...
modelo_de_iowa = DecisionTreeRegressor(random_state = 1)

# Treinar o modelo
modelo_de_iowa.fit(x_treino, y_treino)

# Fazer o teste de precisão do modelo
previsao_modelo_de_iowa = modelo_de_iowa.predict(x_teste)

# erro_medio = mean_absolute_error(y_teste, previsao_modelo_de_iowa)
# r2 = r2_score(y_teste, previsao_modelo_de_iowa)

modelos_treinados = []

for max_de_folhas in [5, 50, 500, 5000]:
    resultado_mae, resultado_r2, modelo_treinado = calcular_mae(max_de_folhas, x_treino, x_teste, y_treino, y_teste)
    modelos_treinados.append(modelo_treinado)
    print(f"Segundo os testes o modelo de {max_de_folhas} folhas teve: ")
    print(f"Erro médio: ${resultado_mae:.2f}") # Significa que, em média, o modelo erra o preço real da casa em cerca de X doláres... mede a diferença média entre o valor previsto e o valor real. Quanto menor esse número, melhor o modelo.
    print(f"Qualidade do modelo (R²): {resultado_r2:.3f}\n") # Essa é a pontuação, indica o quanto o modelo explica as variações dos dados. O valor varia de 0 a 1
...
while True:
    ...
    if resposta == "n":
        ...
        while True:
            print("\033[33mQual modelo você deseja salvar?")
            print("\033[34m[ 1 ] - 5 Folhas\n[ 2 ] - 50 Folhas\n[ 3 ] - 500 Folhas\n[ 4 ] - 5000 Folhas")
            modelo_escolhido = int(input("\033[35mSua Opção: \033[m"))
            if modelo_escolhido > 0 and modelo_escolhido < 5:
                break
            else:
                print("\033[31mErro: Por favor, Digite um modelo válido!\033[m")
        with open("modelo_treinado.pkl", "wb") as arquivo:
            pickle.dump(modelos_treinados[modelo_escolhido - 1], arquivo)
