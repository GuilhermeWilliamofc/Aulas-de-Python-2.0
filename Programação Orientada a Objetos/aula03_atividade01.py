...
class Pessoa:
    def __init__(
        self, nome, altura, idade
    ):  # a atividade não pediu nome mas eu quero colocar, além de mais algumas coisinhas
        self.nome = nome
        self.altura = altura
        self.idade = idade
...
while True:
    while True:
        ...
        if (
            nome == ""
            or nome.isnumeric()
            or any(caracteres in proibidos for caracteres in nome)
        ):
    ...
    while resposta not in "sn":
        try:
            resposta = (
                input("\033[33mDeseja Continuar? [S/N]: \033[m").strip().lower()[0]
            )
        except (ValueError, KeyboardInterrupt):
            pass
